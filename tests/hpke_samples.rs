...
use hmac::HmacReset;
#[cfg(all(feature = "rustcrypto-x25519"))]
//use hpke::HpkeX25519KeyGen;
use hpke::hpke_types::k256_kems::{HpkeAuthKemSecP256k1HkdfSha256, HpkeKemSecP256k1HkdfSha256};
use hpke::{hpke_types::*};

//use k256::Secp256k1;
use kems::{FromKeys, EncodedSizeUser2,Capsulator, DeriveKeyPairFromSeed,GenerateCapsulatorFromSeed};

use kems::eckem::*;
use kems::{Encapsulate,Decapsulate};
use kdfs::hybrid_array::Array;
use hpke::{hpke_kdf::*};
use hpke::{kem_id, kdf_id, aead_id};

use hpke::hpke_types::{sha2_kdfs::HpkeHkdfSha256,sha2_kdfs::HpkeHkdfSha512,p256_kems::HpkeKemP256HkdfSha256, x25519_kems::HpkeKemX25519HkdfSha256};
...
//pub type HpkeCombinerX25519HkdfSha256 = EcCombinerAllPubKeys<x2519_kems::HpkeKemKdfX25519HkdfSha256>;
//pub type HpkeCombinerX25519HkdfSha256 = EcCombinerNoPubKeys<PassThroughKdf>; //<x2519_kems::HpkeKemKdfX25519HkdfSha256>;
...
fn test_rfc_9180_a_1_1 () {
    ...
    use hpke::hpke_types::{sha2_kdfs::LabelledTkdf1, x25519_kems::{HpkeKemKdfX25519HkdfSha256, HpkeKemX25519HkdfSha256}};
    use kdfs::Kdf;
    ...
    // let (encapsulator2, decapsulator2) = HpkeKemX25519HkdfSha256::derive_from_seed(&ikmE.into());
    // assert_eq!(encapsulator2.as_bytes(), skEm.into());

    // let skEm2: Array<u8, U32> = HpkeKemKdfX25519HkdfSha256::derive_secret_other(&ikmE, &[]);
    // assert_eq!(skEm2, skEm);

    let (c0_send, k_send) = encapsulator.encapsulate_deterministic(&ikmE).unwrap();
    assert! ( c0_send.as_ref() == enc );
    assert! ( k_send.as_slice() == shared_secret );
    ...
    let (key2, base_nonce2, exporter_secret2) = sha2_kdfs::HpkeHkdfSha256::derive::<U16, U12, U32>( &kdf, false, &shared_secret, &info, None).unwrap();
    assert! ( key2 == key);
    ...
    let nonce = hex!("56d890e5accaaf011cff4b7d");
    let ct = hex!("f938558b5d72f1a23810b4be2ab4f84331acc02fc97babc53a52ae8218a355a9
            6d8770ac83d07bea87e13c512a");

    //let he_encryptor = HpkeIesX25519Sha256Aes128Gcm::from_encap_key(recipient_public_key);
    let mut pred_rng = PredictableRng::new(&skEm);

    //let (encapped_key,mut cipher_context) = he_encryptor.setup_sender_cipher ( &mut pred_rng, &recipient_public_key, &info, &[], &[]);

    //let (c0_send, ct_send) = he_encryptor.single_shot_seal(&mut pred_rng, &recipient_public_key, Payload{msg: &pt, aad: &aad}, &info, None).unwrap();
    let (c0_send, ct_send) = encryptor.single_shot_seal(&mut pred_rng, Payload{msg: &pt, aad: &aad}, &info, None).unwrap();
    assert! ( c0_send == enc.into());
    assert! ( ct_send == &ct );
    
    let (c0_send2, mut context_send) = encryptor.setup_sender_cipher(&mut pred_rng, &info, None).unwrap();
    ...
    //let decapsulator3 = X25519Decapsulator::<HpkeKemKdfX25519HkdfSha256>::from_key(recipient_secret_key.clone());

    //let he_decryptor = HpkeIesX25519Sha256Aes128Gcm::new_decryptor2(recipient_secret_key);
    let c0_send = c0_send2;
    ...
    let nonce=hex!("56d890e5accaaf011cff4b7c");
    let ct=hex!("af2d7e9ac9ae7e270f46ba1f975be53c09f8d875bdc8535458c2494e8a6eab25
...
fn test_rfc_9180_a_1_4 () {
    ...
    use hpke::hpke_types::{sha2_kdfs::LabelledTkdf1, x25519_kems::HpkeKeyGenKdfX25519HkdfSha256};
    use kems::{kem_with_kdf::{CombinerAllPubKeys, KemAuthWithKdf}, x25519kem::X25519AuthCapsulator};
    ...
    let key_schedule_context = hex!("03 e78d5cf6190d275863411ff5edd0dece5d39fa48e04eec1ed9b71be34729d18c cb6cffde367bb0565ba28bb02c90744a20f5ef37f30523526106f637abb05449" );
    let secret = hex!("5f96c55e4108c6691829aaabaa7d539c0b41d7c72aae94ae289752f056b6cec4");
    ...
    //let recipient_secret_key = x25519_dalek::StaticSecret::random_from_rng(&mut rand_core::OsRng);
    //let recipient_public_key = x25519_dalek::PublicKey::from(&recipient_secret_key);
    //let (skRm1, pkRm1) = HpkeX25519KeyGen::<Hkdf<Sha256>, LabelKem<kem_id::DhKemX25519HkdfSha256>>::derive_keypair_from_seed(&ikmR).unwrap();
    ...
    //let (skSm1, pkSm1) = HpkeX25519KeyGen::<Hkdf<Sha256>, LabelKem<kem_id::DhKemX25519HkdfSha256>>::derive_keypair_from_seed(&ikmS).unwrap();
    ...
    let sender_private_key = x25519_dalek::StaticSecret::from(skSm);
    let recipient_public_key = x25519_dalek::PublicKey::from(pkRm);

    let mut pred_rng = PredictableRng::new(&skEm);

    //let kdf = KemKdfX25519HkdfSha256::default();
    //..let encapsulator = X25519AuthEncapsulator::<EcCombinerAllPubKeys<x2519_kems::HpkeKemKdfX25519HkdfSha256>,U32>::from_keys( recipient_public_key, sender_private_key.clone() );
    let encapsulator = KemAuthWithKdf::<X25519AuthCapsulator, CombinerAllPubKeys, x25519_kems::HpkeKemKdfX25519HkdfSha256, U32 >::encap_from_keys(recipient_public_key, sender_private_key.clone());
    //let (c0_send, k_send) = encapsulator.try_encap(&mut pred_rng, &recipient_public_key).unwrap();
    let (c0_send, k_send) = encapsulator.encapsulate(&mut pred_rng).unwrap();
    assert! ( c0_send.as_ref() == enc );
    assert! ( k_send == shared_secret );

    //let c0_recv =  X25519EncapKey::<U32>::from_bytes(&c0).unwrap();

    let recipient_secret_key = x25519_dalek::StaticSecret::from(skRm);
    let sender_pub = x25519_dalek::PublicKey::from(pkSm);
    //let kdf = KemKdfX25519HkdfSha256::default();
    //let decapsulator = X25519AuthDecapsulator::<EcCombinerAllPubKeys<x2519_kems::HpkeKemKdfX25519HkdfSha256>>::from_keys(sender_pub, recipient_secret_key.clone());
    let decapsulator = KemAuthWithKdf::<X25519AuthCapsulator, CombinerAllPubKeys, x25519_kems::HpkeKemKdfX25519HkdfSha256, U32 >::decap_from_keys(sender_pub, recipient_secret_key.clone());
    
    let k_recv: Array<u8, U32> = decapsulator.decapsulate(&c0_send).unwrap();

    assert!( k_recv == shared_secret );

    //let kdf = LabelledTkdf1::<HmacReset<Sha256>,LabelNone>::default();
    let kdf = LabelledTkdf1::<HmacReset<Sha256>,LabelHpkeV1>::new_with_label::<LabelKdf::<kem_id::DhKemX25519HkdfSha256,kdf_id::HkdfSha256,aead_id::Aes128Gcm>>().into();
    ...
    let pt = hex!("4265617574792069732074727574682c20747275746820626561757479");
    let aad = hex!("436f756e742d30");
    let nonce = hex!("99d8b5c54669807e9fc70df1");
    let ct = hex!("a84c64df1e11d8fd11450039d4fe64ff0c8a99fca0bd72c2d4c3e0400bc14a40
        f27e45e141a24001697737533e");

    //let he_encryptor = HpkeIesAuthPskEncryptX25519Sha256Aes128Gcm::new(sender_private_key.clone());
    let he_encryptor = HpkeAuthIesX25519Sha256Aes128Gcm::auth_encryptor_from_keys(recipient_public_key, sender_private_key);

    let mut pred_rng = PredictableRng::new(&skEm);
    let (c0_send, ct_send) = he_encryptor.single_shot_seal(&mut pred_rng, Payload{msg: &pt, aad:&aad}, &info, Some(Psk{val:&psk, id: &psk_id})).unwrap();
    assert! ( c0_send == enc.into() );
    assert! ( ct_send == ct );

    let (c0_send2, mut ctx_send) = he_encryptor.setup_sender_cipher(&mut pred_rng, &info, Some(Psk{val:&psk, id:&psk_id})).unwrap();
    let ct_send2 = ctx_send.seal ( Payload{msg: &pt, aad:&aad} ).unwrap();
    assert! ( c0_send2 == enc.into());
    assert! ( ct_send2 == ct);

    let he_decryptor = HpkeAuthIesX25519Sha256Aes128Gcm::auth_decryptor_from_keys(recipient_secret_key, sender_pub);
    let pt_2 = he_decryptor.single_shot_open(&c0_send, &info, Payload{msg:&ct, aad:&aad}, Some(Psk{val:&psk, id:&psk_id}) ).unwrap();

    assert! (pt_2 == pt);
    let mut ctx_recv = he_decryptor.setup_receiver_cipher(&c0_send, &info, Some(Psk{val:&psk, id:&psk_id})).unwrap();
    let pt_recv = ctx_recv.open ( Payload{msg:&ct, aad:&aad} ).unwrap();

    //sequence number: 1
    let pt = hex!("4265617574792069732074727574682c20747275746820626561757479");
    let aad = hex!("436f756e742d31");
    let nonce = hex!("99d8b5c54669807e9fc70df0");
    let ct = hex!("4d19303b848f424fc3c3beca249b2c6de0a34083b8e909b6aa4c3688505c05ff
...
#[test]
#[allow(non_snake_case, unused)]
#[cfg(all(feature="rustcrypto-sha2", feature="rustcrypto-x25519", feature="rustcrypto-chacha20poly1305"))]
fn test_rfc_9180_a_2_1 () {
    ...
    let key_schedule_context = hex!("00431df6cd95e11ff49d7013563baf7f11588c75a6611e
        e2a4404a49306ae4cfc5b69c5718a60cc5876c358d3f7fc31ddb598503f67be58ea1
        e798c0bb19eb9796");
    let secret = hex!("5b9cd775e64b437a2335cf499361b2e0d5e444d5cb41a8a53336d8fe402282c6");
    ...
    assert_eq!( encapsulator.as_bytes(), pkRm.into());
    assert_eq!( decapsulator.as_bytes(), skRm.into());
    //let encapsulator = X25519Encapsulator::<HpkeCombinerX25519HkdfSha256,U32>::from_bytes(&pkRm.into());
    let encapsulator = HpkeKemX25519HkdfSha256::from_bytes_encap(&pkRm.into());
    //let decapsulator = X25519Decapsulator::<HpkeCombinerX25519HkdfSha256>::from_bytes(&skRm.into());
    let decapsulator = HpkeKemX25519HkdfSha256::from_bytes_decap(&skRm.into());
   ...
   let pt = hex!("4265617574792069732074727574682c20747275746820626561757479");
   let aad = hex!("436f756e742d30");
   let nonce = hex!("5c4d98150661b848853b547f");
   let ct = hex!("1c5250d8034ec2b784ba2cfd69dbdb8af406cfe3ff938e131f0def8c8b60b4db
   21993c62ce81883d2dd1b51a28");

//    let he_encryptor = HpkeIesEncrypt::</*U32,*/ /*X25519EncapKey<U32>,*/ X25519Encapsulator<KemKdfX25519Sha256<U32>>, /* KemKdfX25519Sha256::<U32>,*/ {kem_id::DHKEM_X25519_HKDF_SHA256}, 
//         Hkdf<Sha256>, {kdf_id::HKDF_SHA256}, chacha20poly1305::ChaCha20Poly1305, {aead_id::CHACHA20_POLY1305}>::new();

    let recipient_public_key = x25519_dalek::PublicKey::from(pkRm);
    let he_encryptor = HpkeIesX25519Sha256ChaCha20Poly1305::encryptor_from_key(recipient_public_key);
    
    //let he_encryptor = HpkeIesX25519Sha256ChaCha20Poly1305::new_encryptor();
    let mut pred_rng = PredictableRng::new(&skEm);
    //let (c0_send, ct_send) = he_encryptor.seal(&mut pred_rng, &recipient_public_key, &aad, &pt, &info, &[], &[]).unwrap();
    let (c0_send, ct_send ) = he_encryptor.single_shot_seal(&mut pred_rng, Payload{msg: &pt, aad: &aad}, &info, None).unwrap();

    println!( "ct_exp ={:02X?}", ct);
    println!( "ct_send={:02X?}", ct_send);
    assert! ( c0_send == enc.into());
    assert! ( ct_send == &ct );

    let recipient_secret_key = x25519_dalek::StaticSecret::from(skRm);
    //let kdf = KemKdfX25519HkdfSha256::default();
    //let decapsulator3 = X25519Decapsulator::<HpkeCombinerX25519HkdfSha256>::from_key(recipient_secret_key.clone());
    //let decapsulator3 = X25519Decapsulator::from_key(recipient_secret_key.clone());
    // let he_decryptor = HpkeIesDecrypt::</*U32,*/ /*X25519EncapKey<U32>,*/ X25519Decapsulator<KemKdfX25519Sha256<U32>>, /*KemKdfX25519Sha256::<U32>,*/ {kem_id::DHKEM_X25519_HKDF_SHA256}, 
    // Hmac<Sha256>, {kdf_id::HKDF_SHA256}, chacha20poly1305::ChaCha20Poly1305, {aead_id::CHACHA20_POLY1305}>::new(decapsulator3);
    // let he_decryptor = HpkeIesDecrypt::</*U32,*/ /*X25519EncapKey<U32>,*/ X25519Decapsulator<KemKdfX25519Sha256<U32>>, _, /*KemKdfX25519Sha256::<U32>,*/ {kem_id::DHKEM_X25519_HKDF_SHA256}, 
    //     Hkdf<Sha256>, {kdf_id::HKDF_SHA256}, chacha20poly1305::ChaCha20Poly1305, {aead_id::CHACHA20_POLY1305}>::new(recipient_secret_key);
    let he_decryptor = HpkeIesX25519Sha256ChaCha20Poly1305::decryptor_from_key(recipient_secret_key);
    //let pt_2 = he_decryptor.open(&c0_send, &info, &aad, &ct, &[], &[] ).unwrap();
    let c0_send = c0_send;
...
#[cfg(all(feature="rustcrypto-sha2", feature="rustcrypto-aes", feature="rustcrypto-p256"))]
#[test]
fn test_rfc_9180_a_3_1 () {
    use hpke::{HpkeEcKeyGen2, hpke_types::{p256_kems::{HpkeKemKdfP256HkdfSha256, HpkeKeyGenKdfP256HkdfSha256}, sha2_kdfs::LabelledTkdf1}};
    ...
    // Start with the pre-written hpke
    // let (skrm3, pkrm3) = hpke::kem::DhP256HkdfSha256::derive_keypair(&ikmr);
    
    // //println! ( "skrm3={:02X?}", skr)
    // assert!( pkrm3.to_bytes().as_slice() == pkrm);
    // assert! ( skrm3.to_bytes().to_vec() == skrm);
    // let (_skrm2, pkrm2) = HpkeKdfHkdfSha256::derive_key_pair::<NistP256,kem_id::DhKemP256HkdfSha256>(&ikmr );
    // let (_skem2, pkem2) = HpkeKdfHkdfSha256::derive_key_pair::<NistP256,kem_id::DhKemP256HkdfSha256>(&ikme );

    // assert_eq!(pkrm2.to_sec1_bytes().as_ref(), &pkrm );
    // assert_eq!(pkem2.to_sec1_bytes().as_ref(), &pkem );


    //let (pkrm3, skrm3) = HpkeKemP256HkdfSha256::derive_from_seed(&Array::from(ikmr));
    //let (pkrm3, skrm3) = HpkeEcdhKem::<NistP256, HpkeKemKdfP256HkdfSha256, U32, /*SeedAsScalar*/HpkeEcKeyGen2<HmacReset<Sha256>, kem_id::DhKemP256HkdfSha256>>::derive_from_seed(&Array::from(ikmr));
    ...
    // println! ( "skrm2={:02X?}", skrm2.to_bytes());
    // println! ( "skrm={:02X?}", skrm);
    // assert!(skrm2.to_bytes().to_vec() == skrm);
    // assert!(pkrm2.to_sec1_bytes().to_vec() == pkrm);
    // let (skem2, pkem2) = HpkeKdfHkdfSha256::<1,1,1>::derive_key_pair::<NistP256>(&ikme, kem_id::DHKEM_P256_HKDF_SHA256);
    // assert!(skem2.to_bytes().to_vec() == skem);
    // assert!(pkem2.to_sec1_bytes().to_vec() == pkem);
 
    // let mut pred_rng = PredictableRng::new(&ikme);
    // let ( encapsulated_key, mut encryption_context ) =
    //         hpke::setup_sender::<hpke::aead::AesGcm128, hpke::kdf::HkdfSha256, hpke::kem::DhP256HkdfSha256, _> 
    //         (&hpke::OpModeS::Base, &pkrm3, &info, &mut pred_rng ).unwrap();

    // assert! ( encapsulated_key.to_bytes().as_slice() == pkem);
    ...
    // Confirm the ik -> sk operations
    // Encapsulate using RustCrypto KEM traits
    //let recipient_priv_scalar = elliptic_curve::NonZeroScalar::<NistP256>::from_repr(skRm.into()).unwrap();
    //let recipient_pub_key2 = p256::PublicKey::from_secret_scalar(&recipient_priv_scalar);
    //let recipient_public_encoded = recipient_pub_key2.to_encoded_point(true);
    //let recipient_pub_key = p256::PublicKey::from_sec1_bytes(&pkRm ).unwrap();
    //p256::EncodedPoint::from_affine_coordinates(&h_x.into(), &h_y.into(), false);
    let mut pred_rng = PredictableRng::new(&skem);

    //let kdf = HpkeKdf::<U32,HmacKdfKemP256Sha256<U32>,_>::new(&pkrm2);
    //let kdf = HpkeKemKdfP256HkdfSha256::default();
    //let kdf = KemKdfP256Sha256::<U32>::new(&[]);
    
    let encapsulator = <HpkeKemP256HkdfSha256 as Capsulator>::Encapsulator::from_bytes(&GenericArray::from_slice(&pkrm));
    
    //let (c0_calc, k_calc ) = encapsulator.try_encap(&mut pred_rng, &pkrm2).unwrap();
    let (c0_calc, k_calc ) = encapsulator.encapsulate(&mut pred_rng).unwrap();

    assert! ( c0_calc.as_ref() == enc );
    println!( "k_calc={:02X?}", k_calc);
    println!( "shared_secret={:02X?}", k_calc);

    assert! ( k_calc == shared_secret );
    

    // let (key_calc, base_nonce_calc, _, exporter_secret_calc) 
    //     = HmacKdfHpke::<Sha256>::key_schedule_s::<KEM_ID, KDF_ID, AEAD_ID,U32,U16,U12> ( 0/*base */, &shared_secret, &info, &[], &[]);

    // assert! ( key_calc.to_vec() == key);
    // assert! ( base_nonce_calc.to_vec() == base_nonce);
    // assert! ( exporter_secret_calc.to_vec() == exporter_secret);
    ...
    let ct = hex! ( "5ad590bb8baa577f8619db35a36311226a896e7342a6d836d8b7bcd2f20b6c7f
    ...
    // let ciphertext2 = encryption_context.seal(&pt, &aad).expect("encryption failed!");
    // assert! ( ciphertext2 == ct );

        
    //let he_encryptor = HpkeIes::<U32, aes_gcm::Aes128Gcm, {aead_id::AES_128_GCM}, X25519EncapKey<U32>, X25519Encapsulator<KemKdfX25519Sha256<U32>>, KemKdfX25519Sha256::<U32>>::new();
    //let he_encryptor = HpkeIesEncrypt::</*U32,*//*EcEncapKeyUncompressed<NistP256, U32>,*/ EcdhEncapsulatorUncompressed2<NistP256,KemKdfP256Sha256<U32>>, /*KemKdfP256Sha256::<U32>,*/ {kem_id::DHKEM_P256_HKDF_SHA256}, 
    //    Hkdf<Sha256>, {kdf_id::HKDF_SHA256}, aes_gcm::Aes128Gcm, {aead_id::AES_128_GCM}, >::new();

    //let he_encryptor = HpkeIesP256Sha256Aes128Gcm::from_encap_key(pkrm2); //::new();
    ...
    //let kdf = KemKdfP256Sha256::<U32>::new(&[]);
    //let decapsulator3 = EcdhDecapsulator2::<NistP256,KemKdfP256Sha256::<U32>>::new_with_params(recipient_secret_key, kdf);
    // let he_decryptor = HpkeIesDecrypt::</*U32,*/ /*X25519EncapKey<U32>,*/ _, /*KemKdfX25519Sha256::<U32>,*/ {kem_id::DHKEM_P256_HKDF_SHA256}, 
    // Hmac<Sha256>, {kdf_id::HKDF_SHA256}, aes_gcm::Aes128Gcm, {aead_id::AES_128_GCM}>::new(decapsulator3);
    //let he_decryptor = HpkeIesDecrypt::</*U32,*/ /*X25519EncapKey<U32>,*/ EcdhDecapsulator2::<NistP256,KemKdfP256Sha256::<U32>>, _, /*KemKdfX25519Sha256::<U32>,*/ {kem_id::DHKEM_P256_HKDF_SHA256}, 
    //    Hkdf<Sha256>, {kdf_id::HKDF_SHA256}, aes_gcm::Aes128Gcm, {aead_id::AES_128_GCM}>::new(recipient_secret_key.clone());
    ...
    //let key_as_ga = elliptic_curve::generic_array::GenericArray::<u8, U16>::from_slice(&key);
    let cipher = <aes_gcm::Aes128Gcm as aes_gcm::KeyInit>::new(&key.into() );
    let payload = Payload {msg:&pt, aad: &aad};
    //let mut calc_nonce = GenericArray::<u8,U12>::from_slice(&base_nonce).clone();
    //let mut calc_nonce = GenericArray::from(base_nonce.into());
    let mut calc_nonce : Array<_, _> = base_nonce.into();
    calc_nonce[11] ^= sequence_number as u8;
    
    // Not correct for large sequence numbers
    let ciphertext = cipher.encrypt(&calc_nonce, payload).unwrap();
    assert! ( ciphertext == ct);
...
fn test_rfc_9180_a_4_1 () {
    ...
    // assert!( pkrm3.to_bytes().as_slice() == pkrm);
    // assert! ( skrm3.to_bytes().to_vec() == skrm);
    // let (skrm2, pkrm2) = HpkeKdfHkdfSha256::derive_key_pair::<NistP256,kem_id::DhKemP256HkdfSha256>(&ikmr );
    // assert!(skrm2.to_bytes().to_vec() == skrm);
    // assert!(pkrm2.to_sec1_bytes().to_vec() == pkrm);
    // let (skem2, pkem2) = HpkeKdfHkdfSha256::derive_key_pair::<NistP256,kem_id::DhKemP256HkdfSha256>(&ikme );
    // assert!(skem2.to_bytes().to_vec() == skem);
    // assert!(pkem2.to_sec1_bytes().to_vec() == pkem);
    ...
    assert_eq!(skem3.as_bytes().as_slice(), &skem );
    // let mut pred_rng = PredictableRng::new(&ikme);
    // let ( encapsulated_key, _encryption_context ) =
    //     hpke::setup_sender::<hpke::aead::AesGcm128, hpke::kdf::HkdfSha256, hpke::kem::DhP256HkdfSha256, _> 
    //     (&hpke::OpModeS::Base, &pkrm3, &info, &mut pred_rng ).unwrap();
    ...
    //let kdf = HpkeKdf::<U32,HmacKdfKemP256Sha256<U32>,_>::new(&pkrm2);
    //let kdf = HpkeKemKdfP256HkdfSha256::default();
    //pub type HpkeEcCombinerP256HkdfSha256 = EcCombinerAllPubKeys<HpkeKemKdf::<Hkdf<Sha256>, kem_id::DhKemP256HkdfSha256>>;
    
    //let encapsulator = EcdhEncapsulatorUncompressed::<NistP256,HpkeEcCombinerP256HkdfSha256,U32>::from_bytes(&GenericArray::from_slice(&pkrm));
    //let encapsulator = EcdhKem::<NistP256, EcCombinerNoPubKeys<PassThroughKdf>, U32, SeedAsScalar>::from_bytes_encap(&GenericArray::from_slice(&pkrm));
    ...
    // let (key_calc, base_nonce_calc, _, exporter_secret_calc) 
    //     = HmacKdfHpke::<Sha512>::key_schedule_s::<KEM_ID, KDF_ID, AEAD_ID,U64,U16,U12> ( 0/*base */, &shared_secret, &info, &[], &[]);

    // assert! ( key_calc.to_vec() == key);
    // assert! ( base_nonce_calc.to_vec() == base_nonce);
    // assert! ( exporter_secret_calc.to_vec() == exporter_secret);
    //let kdf = LabelledTkdf1::<HmacReset<Sha512>,LabelNone>::default();
    ...
    let ct = hex!("d3cf4984931484a080f74c1bb2a6782700dc1fef9abe8442e44a6f09044c8890
    7200b332003543754eb51917ba");

    //let he_encryptor = HpkeIes::<U32, aes_gcm::Aes128Gcm::ChaCha20Poly1305, {aead_id::CHACHA20_POLY1305}, {kem_id::DHKEM_X25519_HKDF_SHA256}, X25519EncapKey<U32>, X25519Encapsulator<KemKdfX25519Sha256<U32>>, KemKdfX25519Sha256::<U32>>::new();
    // let he_encryptor = HpkeIesEncrypt::<EcdhEncapsulatorUncompressed2<NistP256,KemKdfP256Sha256<U32>>, {kem_id::DHKEM_P256_HKDF_SHA256}, 
    //         Hkdf<Sha512>, {kdf_id::HKDF_SHA512}, aes_gcm::Aes128Gcm, {aead_id::AES_128_GCM}>::new();
    //let he_encryptor = HpkeIesEncryptP256Sha512Aes128Gcm::new();
    //let he_encryptor = HpkeIesP256Sha512Aes128Gcm::from_encap_key(pkrm2);
    ...
    //let decapsulator3 = EcdhDecapsulator2::<NistP256,KemKdfP256Sha256::<U32>>::new_with_params(recipient_secret_key, kdf);
    // let he_decryptor = HpkeIesDecrypt::</*U32,*/ /*X25519EncapKey<U32>,*/ _, /*KemKdfX25519Sha256::<U32>,*/ {kem_id::DHKEM_P256_HKDF_SHA256}, 
    //         Hmac<Sha512>, {kdf_id::HKDF_SHA512}, aes_gcm::Aes128Gcm, {aead_id::AES_128_GCM}>::new(decapsulator3);
    //let he_decryptor = HpkeIesDecrypt::<EcdhDecapsulator2::<NistP256,KemKdfP256Sha256::<U32>>, _, /*KemKdfX25519Sha256::<U32>,*/ {kem_id::DHKEM_P256_HKDF_SHA256}, 
    //        Hkdf<Sha512>, {kdf_id::HKDF_SHA512}, aes_gcm::Aes128Gcm, {aead_id::AES_128_GCM}>::new(recipient_secret_key);
...
fn test_rfc_9180_a_4_2 () {
    ...
    let key_schedule_context = hex!("01713f73042575cebfd132f0cc4338523f8eae95c80a74
                ...
                4891c9a2a87a4eb7cdb289ba5e2ecbf8cd2c8498bb4a383dc021454d70d46fcbbad1
                252ef4f9");
    let secret = hex!("ff2051d2128d5f3078de867143e076262ce1d0aecafc3fff3d607f1eaff0
    ...
    //let kdf = HpkeKdf::<U32,HmacKdfKemP256Sha256<U32>,_>::new(&pkrm2);
    //let kdf = HpkeKemKdfP256HkdfSha256::default();
    //pub type HpkeEcCombinerP256HkdfSha256 = HpkeKemEcCombinerHkdfSha256<kem_id::DhKemP256HkdfSha256>;
    //pub type HpkeEcCombinerP256HkdfSha256 = EcCombinerAllPubKeys<HpkeKemKdf::<Hkdf<Sha256>, kem_id::DhKemP256HkdfSha256>>;
    ...
    //let kdf = HpkeKemKdfP256HkdfSha256::default();
    //let decapsulator = EcdhDecapsulatorUncompressed::<_,HpkeKemKdfP256HkdfSha256,U32>::from_key(skrm2);
    //let decapsulator = EcdhDecapsulatorUncompressed::<NistP256,HpkeEcCombinerP256HkdfSha256,U32>::from_bytes(&skRm.into());
...
fn test_rfc_9180_a_4_3 () {
    use aead::inout::block_padding::AnsiX923;
    use hpke::{HpkeEcKeyGen, hpke_types::sha2_kdfs::LabelledTkdf1};
    use kems::kem_with_kdf::{CombinerAllPubKeys, CombinerNoKeys, KemAuthWithKdf, KemAuthWithKdfDecapsulator, KemAuthWithKdfEncapsulator};
    ...
    let key_schedule_context = hex!("025b8a3617af7789ee716e7911c7e77f84cdc4cc46e60f
                b7e19e4059f9aeadc00585e26874d1ddde76e551a7679cd47168c466f6e1f705cc93
                74c192778a34fcd5ca221d77e229a9d11b654de7942d685069c633b2362ce3b3d8ea
                4891c9a2a87a4eb7cdb289ba5e2ecbf8cd2c8498bb4a383dc021454d70d46fcbbad1
                252ef4f9");
    let secret = hex!("9c846ba81ddbbd57bc26d99da6cf7ab956bb735ecd47fe21ed14241c70791b74
    ...
    //let (skRm2, pkRm2) = HpkeKdfHkdfSha256::derive_key_pair::<NistP256,kem_id::DhKemP256HkdfSha256>(&ikmR);
    //let (skSm2, pkSm2) = HpkeKdfHkdfSha256::derive_key_pair::<NistP256,kem_id::DhKemP256HkdfSha256>(&ikmS);

    //let r = EcdhKem::<NistP256, kems::eckem::EcCombinerEphemPubKey<kdfs::ansi_x9_63::X963Kdf<Sha256>>,U16, _, HpkeEcKeyGen<Hkdf<Sha256>,kem_id::DhKemP256HkdfSha256>>::derive_from_seed(&ikmR.into());
    ...
    //let kdf = HpkeKdf::<U32,HmacKdfKemP256Sha256<U32>,_>::new(&pkrm2);
    //let kdf = KemKdfP256Sha256::<U32>::new(&pkRm);
    //let kdf = HpkeKemKdfP256HkdfSha256::default();
    //let encapsulator = EciesEncapsulatorUncompressed2::new(kdf);
    //pub type HpkeEcCombinerP256HkdfSha256 = EcCombinerAllPubKeys<HpkeKemKdf::<Hkdf<Sha256>, kem_id::DhKemP256HkdfSha256>>;
    
    //let encapsulator = EcdhAuthEncapsulatorUncompressed::<_,HpkeEcCombinerP256HkdfSha256,U32>::from_keys(pkRm3.encapsulator.recipient_public, skSm3.decapsulator.recipient_private);
    //let encapsulator = KemAuthWithKdfEncapsulator::<EcdhAuthCapsulatorUncompressed::<_>, CombinerAllPubKeys, HpkeLabelledKdf::<Hkdf<Sha256>, HpkeKemLabel<kem_id::DhKemP256HkdfSha256>>, U32>::from_keys(pkRm3.encapsulator.recipient_public, skSm3.decapsulator.recipient_private);
    let encapsulator = KemAuthWithKdfEncapsulator::<EcdhAuthCapsulatorUncompressed::<_>, CombinerAllPubKeys, KdfForKemUsingHkdf::<Sha256, kem_id::DhKemP256HkdfSha256>, U32>::from_keys(pkRm3.encapsulator.recipient_public, skSm3.decapsulator.recipient_private);
    //let encapsulator = <KemAuthWithKdf::<EcdhAuthCapsulatorUncompressed::<_>, CombinerNoKeys, HpkeKemKdf::<Hkdf<Sha256>, kem_id::DhKemP256HkdfSha256>, U32>::from_keys(pkRm3.encapsulator.recipient_public, skSm3.decapsulator.recipient_private);

    let (c0_calc, k_calc) = encapsulator.encapsulate(&mut pred_rng).unwrap();
    println!( "k_calc={:02X?}", k_calc);
    println!( "c0_calc={:02X?}", c0_calc);
    
    assert! ( c0_calc.as_ref() == enc );
    assert! ( k_calc == shared_secret );

    //let kdf = HpkeKemKdfP256HkdfSha256::default();
    //let decapsulator = EcdhAuthDecapsulator::<_,HpkeEcCombinerP256HkdfSha256,U32,EcUncompressedEncoder<NistP256>>::from_keys(pkSm3.encapsulator.recipient_public, skRm3.decapsulator.recipient_private);
    //let decapsulator = KemAuthWithKdfDecapsulator::<EcdhAuthCapsulator::<_, EcUncompressedEncoder<NistP256>>, CombinerAllPubKeys, HpkeLabelledKdf::<Hkdf<Sha256>, HpkeKemLabel<kem_id::DhKemP256HkdfSha256>>,U32>::from_keys(pkSm3.encapsulator.recipient_public, skRm3.decapsulator.recipient_private);
    let decapsulator = KemAuthWithKdfDecapsulator::<EcdhAuthCapsulator::<_, EcUncompressedEncoder<NistP256>>, CombinerAllPubKeys, KdfForKemUsingHkdf::<Sha256, kem_id::DhKemP256HkdfSha256>,U32>::from_keys(pkSm3.encapsulator.recipient_public, skRm3.decapsulator.recipient_private);
...
#[test]
#[allow(non_snake_case, unused)]
#[cfg(all(feature="rustcrypto-sha2", feature="rustcrypto-p256", feature="rustcrypto-hmac"))]
fn test_rfc_9180_a_4_4 () {
    //use key_derivation::KdfFixed;

    use hpke::hpke_types::sha2_kdfs::LabelledTkdf1;
    use kems::kem_with_kdf::{CombinerAllPubKeys, CombinerNoKeys, KemAuthWithKdf};
    ...
    let key_schedule_context = hex!("03713f73042575cebfd132f0cc4338523f8eae95c80a74
    9f7cf3eb9436ff1c612ca62c37df27ca46d2cc162445a92c5f5fdc57bcde129ca7b1
    f284b0c12297c037ca221d77e229a9d11b654de7942d685069c633b2362ce3b3d8ea
    4891c9a2a87a4eb7cdb289ba5e2ecbf8cd2c8498bb4a383dc021454d70d46fcbbad1
    252ef4f9");
    let secret = hex!("0f9df08908a6a3d06c8e934cd3f5313f9ebccd0986e316c0198bb48bed30
    ...
    type HpkeEcdhAuthP256HkdfSha256Kem = KemAuthWithKdf<EcdhAuthCapsulatorUncompressed<NistP256>, CombinerAllPubKeys, KdfForKemUsingHkdf::<Sha256, kem_id::DhKemP256HkdfSha256>,U32>;
    
    //let kdf = KemKdfP256Sha256::<U32>::new(&pkRm);
    //let kdf = HpkeEcCombinerP256HkdfSha256::default();
    //let encapsulator = EciesEncapsulatorUncompressed2::new(kdf);
    //let encapsulator = EcdhAuthEncapsulatorUncompressed::<_,HpkeEcCombinerP256HkdfSha256,U32>::from_keys(pkRm3.encapsulator.recipient_public, skSm3.decapsulator.recipient_private.clone());
    //let encapsulator = EcdhAuthEncapsulatorUncompressed::<_,HpkeEcCombinerP256HkdfSha256,U32>::from_keys(pkRm3.encapsulator.recipient_public, skSm3.decapsulator.recipient_private.clone());
    let encapsulator = HpkeEcdhAuthP256HkdfSha256Kem::encap_from_keys(pkRm3.encapsulator.recipient_public, skSm3.decapsulator.recipient_private.clone());
    ...
    //let kdf = HpkeKemKdfP256HkdfSha256::default();
    //let decapsulator = EcdhAuthDecapsulator::<_,HpkeEcCombinerP256HkdfSha256,U32,EcUncompressedEncoder<NistP256>>::from_keys(pkSm3.encapsulator.recipient_public, skRm3.decapsulator.recipient_private.clone());
    let decapsulator = HpkeEcdhAuthP256HkdfSha256Kem::decap_from_keys(pkSm3.encapsulator.recipient_public, skRm3.decapsulator.recipient_private.clone());
   ...
   // sequence number: 0
    let pt = hex!("4265617574792069732074727574682c20747275746820626561757479");
    let aad = hex!("436f756e742d30");
    let nonce = hex!("76af62719d33d39a1cb6be9f");
    let ct = hex!("840669634db51e28df54f189329c1b727fd303ae413f003020aff5e26276aaa9
    10fc4296828cb9d862c2fd7d16");


    // let he_encryptor = HpkeIesAuthEncrypt::</*U32,*/ 
    //     EcdhAuthEncapsulatorUncompressed3<NistP256, KemKdfP256Sha256<U32>>, { kem_id::DHKEM_P256_HKDF_SHA256},
    //     //X25519AuthEncapsulator<KemKdfX25519Sha256<U32>>, /*KemKdfP521Sha512::<U64>,*/ {kem_id::DHKEM_X25519_HKDF_SHA256},
    //     Hkdf<Sha512>, {kdf_id::HKDF_SHA512}, 
    //     aes_gcm::Aes128Gcm, {aead_id::AES_128_GCM}>::new(encapsulator.clone());
    ...
    //let he_encryptor = HpkeIesAuthPskEncryptP256Sha512Aes128Gcm::new(skSm2);
    let mut pred_rng = PredictableRng::new(&skEm);
    let (c0_send, ct_send) = he_encryptor.single_shot_seal(&mut pred_rng, Payload{msg: &pt, aad: &aad}, &info, Some(Psk{val:&psk, id:&psk_id})).unwrap();
    //let (c0_send2, ct_send2) = he_encryptor2.single_shot_seal(&mut pred_rng, &pkRm2, Payload{msg: &pt, aad: &aad}, &info, &psk, &psk_id).unwrap();

    println! ( "c0_send={:02X?}\nct     ={:02X?}", c0_send, &ct);

    assert! ( ct_send == ct );

    //let recipient_secret_key = SecretKey::<NistP521>::from_bytes(&skRm3)).unwrap();
    //let kdf = KemKdfP521Sha512::<U64>::new(&[]);
    //let decapsulator3 = EcdhDecapsulator2::<NistP521,KemKdfP521Sha512::<U64>>::new_with_params(skRm3, kdf);
    // let he_decryptor = HpkeIesDecrypt::</*U32,*/ /*X25519EncapKey<U32>,*/ _, /*KemKdfX25519Sha256::<U32>,*/ {kem_id::DHKEM_P521_HKDF_SHA512}, 
    //         Hmac<Sha512>, {kdf_id::HKDF_SHA512}, aes_gcm::Aes256Gcm, {aead_id::AES_256_GCM}>::new(decapsulator3);
    // let he_decryptor = HpkeIesAuthDecrypt::</*U32,*/ /*X25519EncapKey<U32>,*/ 
    //     EciesAuthDecapsulator3::<NistP256, KemKdfP256Sha256::<U32>>, {kem_id::DHKEM_P256_HKDF_SHA256}, 
    //     Hkdf<Sha512>, {kdf_id::HKDF_SHA512}, 
    //     aes_gcm::Aes128Gcm, {aead_id::AES_128_GCM}>::new(skRm2);
    ...
    //type Hkdf22 = Tkdf<HkdfExtract<Hmac<sha2::Sha256>>, HkdfExpand<Hmac<sha2::Sha256>,U32>>;

    // let x = HkdfExtract::<Hmac::<sha2::Sha256>>::derive_salt_secret_fixed(&[], &[]);
    // let r = <Tkdf::<HkdfExtract::<Hmac::<sha2::Sha256>>, HkdfExpand::<Hmac::<sha2::Sha256>>> as TwoStepKdf>::Extract::derive_salt_secret_fixed(&[], &[]);
    // let s = <Hkdf<sha2::Sha256> as TwoStepKdf>::Extract::derive_salt_secret_fixed(&[],&[]);
...
#[test]
#[cfg(all(feature="rustcrypto-sha2", feature="rustcrypto-p256", feature="rustcrypto-chacha20poly1305"))]
fn test_rfc_9180_a_5_1 () {
    ...
    // //let (skrm2, pkrm2) = HpkeKdfHkdfSha512::<1,1,1>::derive_key_pair::<NistP256>(&ikmr, kem_id::DHKEM_P256_HKDF_SHA256);
    ...
    //let kdf = HpkeKdf::<U32,HmacKdfKemP256Sha256<U32>, NistP256>::new(&pkrm2);
    //pub type HpkeEcCombinerP256HkdfSha256 = EcCombinerAllPubKeys<HpkeKemKdf::<Hkdf<Sha256>, kem_id::DhKemP256HkdfSha256>>;
    ...
    // let (key_calc, base_nonce_calc, _, exporter_secret_calc) : (_,_,_,GenericArray<u8,U32>) 
    //     = HmacKdfHpke::<sha2::Sha256>::key_schedule_s::<KEM_ID,KDF_ID,AEAD_ID,_,U32,U12> ( 0/*base */, &shared_secret, &info, &[], &[]);
    ...
    let ct= hex!("6469c41c5c81d3aa85432531ecf6460ec945bde1eb428cb2fedf7a29f5a685b4
            ccb0d057f03ea2952a27bb458b");


    // let he_encryptor = HpkeIesEncrypt::<EcdhEncapsulatorUncompressed2<NistP256,KemKdfP256Sha256<U32>>, {kem_id::DHKEM_P256_HKDF_SHA256}, 
    //         Hkdf<Sha256>, {kdf_id::HKDF_SHA256},chacha20poly1305::ChaCha20Poly1305, {aead_id::CHACHA20_POLY1305}>::new();
    //let he_encryptor = HpkeIesEncryptP256Sha256ChaCha20Poly1305::new();
    //let he_encryptor = HpkeIesP256Sha256ChaCha20Poly1305::from_encap_key(pkrm);
    let he_encryptor = HpkeIesP256Sha256ChaCha20Poly1305::encryptor_from_bytes(&GenericArray::<u8, U65>::from_slice(&pkrm));
    let mut pred_rng = PredictableRng::new(&skem);
    let (c0_send, ct_send) = he_encryptor.single_shot_seal(&mut pred_rng, aead::Payload{msg:&pt, aad:&aad}, &info, None).unwrap();
    ...
    // let he_decryptor = HpkeIesDecrypt::</*U32,*/ /*X25519EncapKey<U32>,*/ _, /*KemKdfX25519Sha256::<U32>,*/ {kem_id::DHKEM_P256_HKDF_SHA256}, 
    //         Hmac<Sha256>, {kdf_id::HKDF_SHA256}, chacha20poly1305::ChaCha20Poly1305, {aead_id::CHACHA20_POLY1305}>::new(decapsulator3);
    // let he_decryptor = HpkeIesDecrypt::</*U32,*/ EcdhDecapsulator2::<NistP256,KemKdfP256Sha256::<U32>>, /*X25519EncapKey<U32>,*/ _, /*KemKdfX25519Sha256::<U32>,*/ {kem_id::DHKEM_P256_HKDF_SHA256}, 
    //          Hkdf<Sha256>, {kdf_id::HKDF_SHA256}, chacha20poly1305::ChaCha20Poly1305, {aead_id::CHACHA20_POLY1305}>::new(recipient_secret_key);
    let he_decryptor = HpkeIesP256Sha256ChaCha20Poly1305::decryptor_from_key(recipient_secret_key);
    let pt_2 = he_decryptor.single_shot_open(&c0_send, &info, Payload{msg:&ct, aad:&aad}, None ).unwrap();
...
#[test]
#[allow(non_snake_case, unused)]
#[cfg(all(feature="rustcrypto-sha2", feature="rustcrypto-p521", feature="rustcrypto-aes", feature="rustcrypto-hmac"))]
fn test_rfc_9180_a_6_1 () {
    ...
    d30e818");
    let key_schedule_context = hex!("0083a27c5b2358ab4dae1b2f5d8f57f10ccccc822a4733
    ...
    e85b09a4");
    let secret = hex!("49fd9f53b0f93732555b2054edfdc0e3101000d75df714b98ce5aa295a37
    ...
    //let (skRm2, pkRm2) = HmacKdfHpkeSha512::derive_key_pair::<NistP521>(&ikmR);
    ...
    //assert! ( pkRm2 == pkRm3);
    assert! ( pkRm3 == skRm3.public_key());
    //let (skSm2, pkSm2) = HmacKdfHpkeSha512::derive_key_pair::<NistP521>(&ikmS);

    let mut pred_rng = PredictableRng::new(&skEm);

    //let kdf = HpkeKdf::<U32,HmacKdfKemP256Sha256<U32>,_>::new(&pkrm2);
    //let kdf = KemKdfP256Sha256::<U32>::new(&pkRm);
    //let kdf = KdfEc2::<KemKdfP521HkdfSha512, NistP521, false>::default();
    //let kdf = KemKdf2::<Hkdf<Sha512>, NistP521, {kem_id::DHKEM_P521_HKDF_SHA512}>::default();
    //let kdf = EcCombinerAllPubKeys::<HpkeKemKdf::<Hkdf<Sha512>, kem_id::DhKemP521HkdfSha512>>::default();

    //let encapsulator = EciesEncapsulatorUncompressed2::new(kdf);
    //let encapsulator = EcdhEncapsulatorUncompressed::<NistP521,EcCombinerAllPubKeys::<HpkeKemKdf::<Hkdf<Sha512>, kem_id::DhKemP521HkdfSha512>>, U64>::from_bytes(GenericArray::from_slice(&pkRm));
    let encapsulator = HpkeKemP521HkdfSha512::from_bytes_encap(GenericArray::from_slice(&pkRm));
    ...
    println!( "k_calc={:02X?}", k_calc);
    println!( "k_ref ={:02X?}", shared_secret);

    assert! ( k_calc == shared_secret );

    //let kdf = KemKdfP521Sha512::default();
    let mut hkdf = kdfs::iso11770_6::Tkdf1::<HmacReset<Sha512>,u8>::default();

    //let mut kdf = KemKdf2::<Hkdf<sha2::Sha512>,NistP521,{kem_id::DHKEM_P521_HKDF_SHA512}>::default();
    //let mut kdf = EcCombinerAllPubKeys::<HpkeKemKdf::<Hkdf<sha2::Sha512>, {kem_id::DHKEM_P521_HKDF_SHA512}>>::default();

    //let x: GenericArray<u8, U8> = KdfFixed::derive_self_secret_other_fixed(&mut hkdf, &[], &[]);


    //let decapsulator = EcdhDecapsulatorUncompressed::<_,EcCombinerAllPubKeys::<HpkeKemKdf::<Hkdf<sha2::Sha512>, kem_id::DhKemP521HkdfSha512>>,U64>::from_key(skRm3.clone());
    let decapsulator = HpkeKemP521HkdfSha512::from_bytes_decap(GenericArray::from_slice(&skRm));
    let k_calc2 = decapsulator.decapsulate(&c0_calc).unwrap();
    assert! ( k_calc2 == shared_secret );

    //let x = Array::default();
    let (encryptor, decryptor) = HpkeIesP521Sha512Aes256Gcm::derive_pair_from_seed(&ikmR).unwrap();
    assert_eq! ( decryptor.decapsulator.as_bytes().as_slice(), skRm);
    assert_eq! ( encryptor.encapsulator.as_bytes().as_slice(), pkRm);
    ...
    let pt = hex!("4265617574792069732074727574682c20747275746820626561757479");
    let aad = hex!("436f756e742d30");
    let nonce = hex!("55ff7a7d739c69f44b25447b");
    let ct = hex!("170f8beddfe949b75ef9c387e201baf4132fa7374593dfafa90768788b7b2b20
    0aafcc6d80ea4c795a7c5b841a");


    // let he_encryptor = HpkeIesEncrypt::</*U32,*/ /*EcEncapKeyUncompressed<NistP521, U64>, */
    //             EcdhEncapsulatorUncompressed2<NistP521,KemKdfP521Sha512<U64>>, /*KemKdfP521Sha512::<U64>,*/ {kem_id::DHKEM_P521_HKDF_SHA512},
    //             Hkdf<Sha512>, {kdf_id::HKDF_SHA512}, 
    //             aes_gcm::Aes256Gcm, {aead_id::AES_256_GCM}>::new();
    //let he_encryptor = HpkeIesEncryptP521Sha512Aes256Gcm::new();
    let he_encryptor = HpkeIesP521Sha512Aes256Gcm::encryptor_from_key(pkRm3);
    let mut pred_rng = PredictableRng::new(&skEm);
    let (c0_send, ct_send) = he_encryptor.single_shot_seal(&mut pred_rng, Payload{msg: &pt, aad: &aad}, &info, None).unwrap();
    ...
    // let he_decryptor = HpkeIesDecrypt::</*U32,*/ /*X25519EncapKey<U32>,*/ _, /*KemKdfX25519Sha256::<U32>,*/ {kem_id::DHKEM_P521_HKDF_SHA512}, 
    //         Hmac<Sha512>, {kdf_id::HKDF_SHA512}, aes_gcm::Aes256Gcm, {aead_id::AES_256_GCM}>::new(decapsulator3);
    // let he_decryptor = HpkeIesDecrypt::</*U32,*/ /*X25519EncapKey<U32>,*/ EcdhDecapsulator2::<NistP521,KemKdfP521Sha512::<U64>>,_, /*KemKdfX25519Sha256::<U32>,*/ {kem_id::DHKEM_P521_HKDF_SHA512}, 
    //         Hkdf<Sha512>, {kdf_id::HKDF_SHA512}, aes_gcm::Aes256Gcm, {aead_id::AES_256_GCM}>::new(skRm3);
    let he_decryptor = HpkeIesP521Sha512Aes256Gcm::decryptor_from_key(skRm3);
    let pt_2 = he_decryptor.single_shot_open(&c0_send, &info, Payload{msg:&ct, aad:&aad}, None ).unwrap();
...
#[test]
#[allow(non_snake_case, unused)]
#[cfg(all(feature="rustcrypto-sha2", feature="rustcrypto-p521", feature="rustcrypto-aes"))]
fn test_rfc_9180_a_6_3 () {
    ...
    use hpke::hpke_types::p521_kems::{HpkeAuthKemP521HkdfSha512, HpkeKemKdfP521HkdfSha512};
    //pub type HpkeEcCombinerP521HkdfSha512 = EcCombinerAllPubKeys<HpkeKemKdfP521HkdfSha512>;
   ...
   let skEm = hex!("0185f03560de87bb2c543ef03607f3c33ac09980000de25eabe3b224312946
   ...
   c00f30a");
   let key_schedule_context = hex!("0283a27c5b2358ab4dae1b2f5d8f57f10ccccc822a4733
   26f543f239a70aee46347324e84e02d7651a10d08fb3dda739d22d50c53fbfa8122b
   aacd0f9ae5913072ef45baa1f3a4b169e141feb957e48d03f28c837d8904c3d67753
   08c3d3faa75dd64adfa44e1a1141edf9349959b8f8e5291cbdc56f62b0ed6527d692
   e85b09a4");
   let secret = hex!("56b7acb7355d080922d2ddc227829c2276a0b456087654b3ac4b53828bd3
   ...
   9cfb7fe47");
    ...
    // let (skRm2, pkRm2) = HpkeKdfHkdfSha512::derive_key_pair::<NistP521,kem_id::DhKemP256HkdfSha256>(&ikmR);
    // let (skSm2, pkSm2) = HpkeKdfHkdfSha512::derive_key_pair::<NistP521, kem_id::DhKemP256HkdfSha256>(&ikmS);
    ...
    //let kdf = HpkeKdf::<U32,HmacKdfKemP256Sha256<U32>,_>::new(&pkrm2);
    //let kdf = KemKdfP256Sha256::<U32>::new(&pkRm);
    let kdf = HpkeKemKdfP521HkdfSha512::default();
    //let encapsulator = EciesEncapsulatorUncompressed2::new(kdf);
    //let encapsulator = EcdhAuthEncapsulatorUncompressed::<_,HpkeEcCombinerP521HkdfSha512,U32>::from_keys(pkRm3, skSm3.clone());
    ...
    assert! ( c0_calc.as_ref() == enc );
    //assert! ( k_calc.as_bytes() == shared_secret );

    let kdf = HpkeKemKdfP521HkdfSha512::default();
    //let decapsulator = EcdhAuthDecapsulator::<_,HpkeEcCombinerP521HkdfSha512,U64,EcUncompressedEncoder<NistP521>>::from_keys(pkSm3, skRm3.clone());
    ...
    let pt = hex!("4265617574792069732074727574682c20747275746820626561757479");
    let aad = hex!("436f756e742d30");
    let nonce = hex!("9752b85fe8c73eda183f9e80");
    let ct = hex!("0116aeb3a1c405c61b1ce47600b7ecd11d89b9c08c408b7e2d1e00a4d64696d1
    2e6881dc61688209a8207427f9");

    // let he_encryptor = HpkeIesAuthEncrypt::</*U32,*/ /*EcEncapKeyUncompressed<NistP521, U64>, */
    //             EcdhAuthEncapsulatorUncompressed3<NistP521,KemKdfP521Sha512<U64>>, /*KemKdfP521Sha512::<U64>,*/elliptic_curve::SecretKey<NistP521>, {kem_id::DHKEM_P521_HKDF_SHA512},
    //             Hkdf<Sha512>, {kdf_id::HKDF_SHA512}, 
    //             aes_gcm::Aes256Gcm, {aead_id::AES_256_GCM}>::new(skSm3);
    ...
    // let he_decryptor = HpkeIesDecrypt::</*U32,*/ /*X25519EncapKey<U32>,*/ _, /*KemKdfX25519Sha256::<U32>,*/ {kem_id::DHKEM_P521_HKDF_SHA512}, 
    //         Hmac<Sha512>, {kdf_id::HKDF_SHA512}, aes_gcm::Aes256Gcm, {aead_id::AES_256_GCM}>::new(decapsulator3);
    //let he_decryptor = HpkeIesAuthDecrypt::</*U32,*/ /*X25519EncapKey<U32>,*/ EciesAuthDecapsulator3::<NistP521,KemKdfP521Sha512::<U64>>, elliptic_curve::SecretKey<NistP521>, {kem_id::DHKEM_P521_HKDF_SHA512}, 
    //         Hkdf<Sha512>, {kdf_id::HKDF_SHA512}, aes_gcm::Aes256Gcm, {aead_id::AES_256_GCM}>::new(skRm3);
...
fn test_rfc_9180_a_6_4 () {
    use hpke::hpke_types::{p521_kems::{HpkeAuthKemP521HkdfSha512, HpkeKemKdfP521HkdfSha512, HpkeKemP521HkdfSha512}, sha2_kdfs::LabelledTkdf1};
    //pub type HpkeEcCombinerP521HkdfSha512 = EcCombinerAllPubKeys<HpkeKemKdfP521HkdfSha512>;
    ...
    3b5b945");
    let key_schedule_context = hex!("0324497637cf18d6fbcc16e9f652f00244c981726f293b
    ...
    08c3d3faa75dd64adfa44e1a1141edf9349959b8f8e5291cbdc56f62b0ed6527d692
    e85b09a4");
    let secret = hex!("50a57775958037a04098e0054576cd3bc084d0d08d29548ba4befa5676b9
    ...
    //let (skrmA, pkrm3) = hpke::kem::DhP521HkdfSha512::derive_keypair(&ikmr);
    //println!( "skrmA={:2X?}", skrmA);

    // let (skRm2, pkRm2) = HpkeKdfHkdfSha512::derive_key_pair::<NistP521,kem_id::DhKemP521HkdfSha512>(&ikmR);
    // let (skSm2, pkSm2) = HpkeKdfHkdfSha512::derive_key_pair::<NistP521,kem_id::DhKemP521HkdfSha512>(&ikmS);
    ...
    let skRm3 = elliptic_curve::SecretKey::<NistP521>::from_slice(&skRm).unwrap();
    assert! ( skRm2.decapsulator.recipient_private == skRm3);
    let pkRm3 = elliptic_curve::PublicKey::<NistP521>::from_sec1_bytes(pkRm.as_ref()).unwrap();
    let skSm3 = elliptic_curve::SecretKey::<NistP521>::from_slice(&skSm).unwrap();
    //assert! ( skRm2 == skRm3);
    let pkSm3 = elliptic_curve::PublicKey::<NistP521>::from_sec1_bytes(pkSm.as_ref()).unwrap();
    ...
    let pt=hex!("4265617574792069732074727574682c20747275746820626561757479");
    let aad=hex!("436f756e742d30");
    let nonce=hex!(" d9c64ec8deb8a0647fafe8ff");
    let ct=hex!(" 942a2a92e0817cf032ce61abccf4f3a7c5d21b794ed943227e07b7df2d6dd92c
            9b8a9371949e65cca262448ab7");

    //let he_encryptor = HpkeIesAuthPskEncryptP521Sha512Aes256Gcm::new(skSm3);
    let he_encryptor = HpkeAuthIesP521Sha512Aes256Gcm::auth_encryptor_from_keys(pkRm3, skSm3 );
    let mut pred_rng = PredictableRng::new(&skEm);
    let (c0_send, ct_send) = he_encryptor.single_shot_seal(&mut pred_rng, Payload{ msg: &pt, aad: &aad}, &info, Some(Psk{val:&psk, id:&psk_id})).unwrap();
...
// aead_id: 65535
    ...
    let shared_secret=hex!("e81716ce8f73141d4f25ee9098efc968c91e5b8ce52ffff59d64039e82918b66");
    let key_schedule_context=hex!("009bd09219212a8cf27c6bb5d54998c5240793a70ca0a8
    92234bd5e082bc619b6a3f4c22aa6d9a0424c2b4292fdf43b8257df93c2f6adbf6dd
    c9c64fee26bdd292");
    let secret=hex!("04d64e0620aa047e9ab833b0ebcd4ff026cefbe44338fd7d1a93548102ee01af");
    // let key:
    // let base_nonce:
    let exporter_secret=hex!("79dc8e0509cf4a3364ca027e5a0138235281611ca910e435e8ed58167c72f79b");
    ...
    let mut pred_rng = PredictableRng::new(&skEm);
    //let (c0_send2, mut context_send) = he_encryptor.setup_sender(&mut pred_rng, &recipient_public_key, &info).unwrap();
    
    let exporter_context=hex!("");
    //let L: 32
    let exported_value=hex!("7a36221bd56d50fb51ee65edfd98d06a23c4dc87085aa5866cb7087244bd2a36");

    let (enc_calc, calc_exported_value) = he_encryptor.single_shot_sender_export::<_,U32>(&mut pred_rng, &exporter_context, &info, None).unwrap();
...
fn test_rfc_9180_a_7_2 () {
    ...
    use hpke::hpke_types::aes_aeads::ExportOnlyAead;
    let info = hex!("4f6465206f6e2061204772656369616e2055726e");
    let ikmE = hex!("c51211a8799f6b8a0021fcba673d9c4067a98ebc6794232e5b06cb9febcbbdf5");
    let pkEm = hex!("d3805a97cbcd5f08babd21221d3e6b362a700572d14f9bbeb94ec078d051ae3d");
    let skEm = hex!("1d72396121a6a826549776ef1a9d2f3a2907fc6a38902fa4e401afdb0392e627");
    ...
    let shared_secret = hex!("024573db58c887decb4c57b6ed39f2c9a09c85600a8a0ecb11cac24c6aaec195");
    let key_schedule_context = hex!("01446fb1fe2632a0a338f0a85ed1f3a0ac475bdea2cd72
    f8c713b3a46ee737379a3f4c22aa6d9a0424c2b4292fdf43b8257df93c2f6adbf6dd
    c9c64fee26bdd292");
    let secret = hex!("638b94532e0d0bf812cf294f36b97a5bdcb0299df36e22b7bb6858e3c113080b");
    //let key:
    //let base_nonce:
    let exporter_secret = hex!("04261818aeae99d6aba5101bd35ddf3271d909a756adcef0d41389d9ed9ab153");
    ...
    let (enc_calc, calc_exported_value) = he_encryptor.single_shot_sender_export::<_,U32>(&mut pred_rng, &exporter_context, &info, Some(Psk{val:&psk, id:&psk_id})).unwrap();
    println! ( "calc_exported_value={:02X?}", calc_exported_value);
    assert! ( calc_exported_value == exported_value);
    ...
    //let recv_exported_value = exporter_ctx2.export(&exporter_context);
    ...
    let recv_exported_value = exporter_ctx2.export::<U32>(&kdf, &exporter_context).unwrap();
    assert! ( recv_exported_value == exported_value);
...
fn test_rfc_9180_a_7_3 () {
    ...
    let skEm = hex!("83d3f217071bbf600ba6f081f6e4005d27b97c8001f55cb5ff6ea3bbea1d9295");
    let ikmR = hex!("fc9407ae72ed614901ebf44257fb540f617284b5361cfecd620bafc4aba36f73");
    ...
    let ikmS = hex!("2ff4c37a17b2e54046a076bf5fea9c3d59250d54d0dc8572bc5f7c046307040c");
    ...
    let shared_secret = hex!("e204156fd17fd65b132d53a0558cd67b7c0d7095ee494b00f47d686eb78f8fb3");
    let key_schedule_context = hex!("02
    ...
    let secret = hex!("355e7ef17f438db43152b7fb45a0e2f49a8bf8956d5dddfec1758c0f0eb1b5d5");
    // let key:
    // let base_nonce:
    let exporter_secret = hex!("276d87e5cb0655c7d3dad95e76e6fc02746739eb9d968955ccf8a6346c97509e");

    let recipient_public_key = x25519_dalek::PublicKey::from(pkRm);
    let recipient_secret_key = x25519_dalek::StaticSecret::from(skRm);
    let sender_secret_key = x25519_dalek::StaticSecret::from(skSm);
    let sender_public_key = x25519_dalek::PublicKey::from(pkSm);
    //let he_encryptor = HpkeIesAuthEncryptX25519Sha256ExportOnly::new(sender_secret_key);
    ...
    let mut pred_rng = PredictableRng::new(&skEm);
    let (enc_calc, exporter_ctx) = he_encryptor.setup_sender_export(&mut pred_rng, &info, None).unwrap();
    assert! ( &enc_calc.as_ref() == &enc);
    ...
    //let kdf = LabelledTkdf1::<HmacReset<Sha256>,LabelNone>::new_with_label(&HpkeKdfLabel::<HpkeKemX25519HkdfSha256,HpkeHkdfSha256,ExportOnlyAead>::LABEL);
    let kdf = LabelledTkdf1::<HmacReset<Sha256>,LabelHpkeV1>::new_with_label::<LabelKdf::<kem_id::DhKemX25519HkdfSha256,kdf_id::HkdfSha256,aead_id::ExportOnly>>().into();
    
    
    let calc_exported_value = exporter_ctx.export::<U32>(&kdf,&exporter_context).unwrap();
    assert! ( calc_exported_value == exported_value);
...
fn test_rfc_9180_a_7_4 () {
    ...
    let info = hex!("4f6465206f6e2061204772656369616e2055726e");
    let _ikmE = hex!("94efae91e96811a3a49fd1b20eb0344d68ead6ac01922c2360779aa172487f40");
    let _pkEm = hex!("81cbf4bd7eee97dd0b600252a1c964ea186846252abb340be47087cc78f3d87c");
    let skEm = hex!("a2b43f5c67d0d560ee04de0122c765ea5165e328410844db97f74595761bbb81");
    let _ikmR = hex!("4dfde6fadfe5cb50fced4034e84e6d3a104aa4bf2971360032c1c0580e286663");
    ...
    let _ikmS = hex!("26c12fef8d71d13bbbf08ce8157a283d5e67ecf0f345366b0e90341911110f1b");
    ...
    let he_decryptor = HpkeAuthIesX25519Sha256ExportOnly::auth_decryptor_from_keys(recipient_secret_key, sender_public_key);
    let mut pred_rng = PredictableRng::new(&skEm);
    let (enc_calc, send_exporter_ctx) = he_encryptor.setup_sender_export(&mut pred_rng, &info, Some(Psk{val:&psk, id:&psk_id})).unwrap();
    assert! ( &enc_calc.as_ref() == &enc);
    ...
    let send_exported_value = send_exporter_ctx.export::<U32>(&kdf, &exporter_context).unwrap();
    assert! ( send_exported_value == exported_value);
    let recv_exported_value = recv_exporter_ctx.export::<U32>(&kdf, &exporter_context).unwrap();
    assert! ( recv_exported_value == exported_value);

    let exporter_context = hex!("00");
    let exported_value = hex!("7346bb0b56caf457bcc1aa63c1b97d9834644bdacac8f72dbbe3463e4e46b0dd");
    let calc_exported_value = send_exporter_ctx.export::<U32>(&kdf, &exporter_context).unwrap();
    assert! ( calc_exported_value == exported_value);
    let recv_exported_value = recv_exporter_ctx.export::<U32>(&kdf, &exporter_context).unwrap();
    assert! ( recv_exported_value == exported_value);
    ...
    let calc_exported_value = send_exporter_ctx.export::<U32>(&kdf, &exporter_context).unwrap();
    assert! ( calc_exported_value == exported_value);
    let calc_exported_value = recv_exporter_ctx.export::<U32>(&kdf, &exporter_context).unwrap();
    assert! ( calc_exported_value == exported_value);
}
...
fn test_p256k1_b_1_2 () {
    ...
    let skSm= hex!("040fc95447fff5a811321da69ff4655d185d58edef93453ab23dfb1be2f02702");
    let pkSm = hex!("04a3935d9f2ea9c4b23cdf49f4761625b2acbc1fc89532fe2c3af9d1b1c61b9f
                167f61ba6125d47151df26e2ecfa851bd79719c99ff354c9b9e9619f25cb6ba6d7");
...
fn test_p256k1_b_2_2 () {
    ...
    let skSm= hex!("45ea3cf6c4fcf5d9874b58f3d7a518584e4e5349756b41d79f76fdbd280259f8");
    let pkSm = hex!("04376203ae7189b010cf97c5df7f8451c836bc4bfe9572d62c88858e1fb58179
                9c762a0157f5f15055c91da4ece1bd536d28cd2cdffd233ee9632b3f8a9c237861");
...
fn test_p256k1_b_3_2 () {
    ...
    let skSm= hex!("38aca581ad6a6a202fa89ac49f89650fac018b7f1d724a72040fea497ed95b84");
    let pkSm = hex!("04a6e334bb434dcf340fa2a8267ed828b23632de1f346b8acd7a5b8e83b9bc3f
                58bbfabfc27dad4cbc30230de97bada0568c73f1ee877a885f5a3754bfc2287c84");
